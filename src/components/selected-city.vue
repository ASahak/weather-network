    ...
    <div>
        ...
        <div class="swiperSlide" ref="widthMainSlider">
            ...
            <div class="main_Slides" ref="MainSlider" :style="{height:heightBadge+'px', width:commonBadgesCount*SliderItemsWidth+'px', transform: 'translate('+(SliderTranslate)+'px)'}">
                <template v-for="(item, index) of changesData">
                  <div :class="{'activeItem':addClassSelectedItem}" :key="index" class="slideWrap" ref="HeightBadge" v-if="index < commonBadgesCount" :style="{width:SliderItemsWidth+'px'}" @click="showMoreWeatherBottom(index)">
        ...
        <div v-if="BooleanMultipleItemWeatherClick" class="moreDescMultipleDay">
          ...
          <ul type="none">
            ...
            <li v-for="(item, index) of bottomArrayData" :key="index">
    ...
    filterDays(dayCount, th){
      if(dayCount == "today"){
        ...
        let arrayDay = [], 
            ...
            Dayresult = [],
            badgeResult = [];
        const createObj = (n, nightIndex, dayIndex) => {
          badgeResult.push({
              multipleDay:true,
